    ...
    <section className="py-16 md:py-24 bg-white">
      <div className="container mx-auto px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 md:mb-16 gap-8">
          ...
          >
            {["All", "Training", "Printing", "Travels"].map((tab) => (
              <button 
                key={tab}
                onClick={() => setActiveFilter(tab)}
                className={`px-5 md:px-6 py-2 rounded-full font-bold text-xs md:text-sm transition-all ${activeFilter === tab ? 'bg-primary text-white shadow-md' : 'bg-off-white text-primary hover:bg-navy hover:text-white'}`}
              >
                {tab}
              </button>
            ))}
