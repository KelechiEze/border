        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12 mb-12 md:mb-16">
          <div>
            <div className="flex items-center space-x-2 mb-6 md:mb-8">
              <div className="w-8 h-8 bg-dark-teal rounded-sm flex items-center justify-center">
                <div className="w-4 h-4 bg-lime rotate-45" />
              </div>
            ...
            <div className="flex space-x-4">
              {[Instagram, Facebook, Twitter].map((Icon, i) => (
                <a key={i} href="#" className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center hover:bg-lime hover:text-primary transition-all">
                  <Icon size={20} />
                </a>
              ))}
            </div>
          ...
          <div>
            ...
            <ul className="space-y-3 md:space-y-4 text-white/60 text-sm md:text-base">
              {['Home', 'About Us', 'Our Services', 'Portfolio', 'Contact'].map((link) => (
                <li key={link}><a href="#" className="hover:text-lime transition-colors">{link}</a></li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3 md:space-y-4 text-white/60 text-sm md:text-base">
              {['Leadership Training', 'Digital Literacy', 'Custom Apparel', 'Corporate Branding', 'Hajj & Umrah'].map((service) => (
                <li key={service}><a href="#" className="hover:text-lime transition-colors">{service}</a></li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-4 md:space-y-6">
              <li className="flex items-start">
                <MapPin className="text-lime mr-4 shrink-0" size={18} md:size={20} />
                <span className="text-white/60 text-sm md:text-base">123 Business Avenue, <br /> London, UK</span>
              </li>
              <li className="flex items-center">
                <Phone className="text-lime mr-4 shrink-0" size={18} md:size={20} />
                <span className="text-white/60 text-sm md:text-base">07950 952103</span>
              </li>
              <li className="flex items-center">
                <Mail className="text-lime mr-4 shrink-0" size={18} md:size={20} />
